...
import { useTranslation, LANGUAGES, type Locale } from "@/providers/i18n-provider";
		...
		<header className="sticky top-0 z-20 flex h-14 items-center justify-between border-b border-slate-100 bg-white/90 px-4 backdrop-blur dark:border-zinc-800 dark:bg-zinc-950/90">
			<div className="flex items-center gap-2">
				...
				<nav className="hidden md:flex items-center text-xs text-slate-500 dark:text-slate-400 gap-2">
					<span className="hover:text-slate-900 dark:hover:text-white cursor-pointer">{t("common.chats")}</span>
					<span className="text-slate-300">/</span>
					<span className="font-medium text-slate-900 dark:text-white">{t("common.newChat")}</span>
				</nav>
				...
				<DropdownMenu>
					...
					<DropdownMenuContent align="end">
						{LANGUAGES.map((lang) => (
							...
							>
								{locale === lang.code && <Check className="mr-2 h-3 w-3" />}
								{lang.label}
							</DropdownMenuItem>
						))}
					</DropdownMenuContent>
